...
\begin{proof} 
    The Lemma is a direct consequence of \cite[Proposition 3.17]{Porta_Yu_Derived_non-archimedean_analytic_spaces} (iii).
\end{proof}
...
%        \]
%    in the derived stable \infcat $\Mod_A$.
%    Moreover, \cite[\S 9, Proposition 5.1.3]{Gaitsgory_Study_II} implies that the natural deformation of the (spectrum of) $g \colon B \to A$ is stable
%    under sifted colimits. For each $m \ge 0$, consider $P_\bullet^{(m)}$ the derived global sections of the $m$-th piece of the Hodge filtration applied componentwise to $P_\bullet$, and
...
%        \] 
%    in $\Mod_{A}$, and similarly
%        \[
%            \colim_{\bDelta \op} \bbL_{(P_\bullet/ I_\bullet)^{(m)}/ P_\bullet^{(m)}} \simeq \bbL_{A^{(m)}/ B},  
%        \]
%    as $A^{(m)}$-modules. As in the proof of (3) \cref{prop:algebraic_properties_of_deformation} it follows that, for each $[n] \in \bDelta$, we have natural morphisms
%        \begin{equation} \label{eq:Hodge_filtration_under_sifted_colimits}
%            \bbL_{(P_{[n]} / I_{[n]})^{(m)}/ P_{[n]}} \to \Sym^{m+1}(\bbL_{(P_{[n]}/ I_{[n]})/ P_{[n]}}[-1])[1],
%        \end{equation}
%    which identify the right hand side with the $n$-th graded piece of a natural (decreasing) filtration on $\bbL_{(P_{[n]}/ I_{[n]})/ P_{[n]}}$.
%    By the naturality of \cite[\S 9, Proposition 5.1.3]{Gaitsgory_Study_II} we obtain that the geometric realization of
%    \eqref{eq:Hodge_filtration_under_sifted_colimits} corresponds to a morphism 
%        \[
%            \bbL_{A^{(m)}/ B} \to \Sym^{m+1}(\bbL_{A/ B}[-1])[1],
%        \]
%    identifying the right hand side with the $n$-th graded piece of a (decreasing) filtration on $\bbL_{A^{(m)}/ B}$. By the fact that the above filtrations are induced by (geometrical)
%    left-lax $\bbA^1_k$-actions under the functor $\Spec$, we obtain by naturality
%    of \cite[\S 9, Theorem 5.1.2]{Gaitsgory_Study_II} that the latter corresponds to the Hodge filtration corresponding to
%    the morphism $B \to A$. In particular, the sifted colimit of the composites
%        \[
...
%
%    Let us now prove the assertion in general. \cite[\S II. 4, Theorem 4]{Quillen_Homotopical_1967} implies that the \emph{simplicial} category of pairs
%    $(A, I)$ where $A$ is a simplicial $k$-algebra and $I$ a simplicial ideal of $A$ admits a simplicial model structure such that:
...
%    and $J_n$ an ideal defined by a subset $Y_n \subseteq X_n$, both preserved by degeneracies and the relative situation.
%    If $f \colon B \to A$ is a local complete intersection then we can represent $f$ by a pair of the form $(B', I')$ where $B'$ is a simplicial $k$-algebra
%    and $I' \subseteq B'$ a regular ideal term-wise. Then we can identify $\bbL_{B/ X}$ with the geometric realization
%        \[
%            \bbL_{B/ A} \simeq \colim_{\bDelta \op} I' / (I')^2,  
%        \]
%    in $\Mod_B$. The argument now follows as in the previous case.
\end{proof}     
...
%    we deduce the statement in the general case by the local complete intersection one by observing that $(P_\bullet/ I_\bullet)^{(n)} \simeq P_\bullet / I_\bullet^n$
%    together with the fact that
%        \[
%            B/ \mathrm{Fil}_H^n \simeq \colim_{\bDelta \op} (P_\bullet / I_\bullet)^\bullet,
%        \]
%    due to \cref{rem:sifted_colimits_and_Hodge_filtration}.
%    Again by the local complete intersection in the discrete case we deduce a chain of natural equivalences
%        \begin{align*}
%            \dR_{B/ A} / \mathrm{Fil}^n_H & \simeq \colim_{\bDelta \op} \dR_{P_\bullet/ (P_\bullet/ I_\bullet)} / \mathrm{Fil}^n_H \\
%                                          & \simeq \colim_{\bDelta \op} P_\bullet/ I^n_\bullet.
%        \end{align*}
%    By the previous considerations, we further conclude the existence of a natural equivalence of the form
%        \[
%            B/ \mathrm{Fil}_H^n \simeq \dR_{B/ A} / \mathrm{Fil}_H^n,
%        \]
%    as desired.
%\end{proof}
...
\end{proof}
%    We first observe that the relative analytification functor commutes with filtered colimits of derived $k$-stacks, as it is defined as the composite of the left Kan extension of the functor $(-)\an_Y \colon \dAff_k \to \dAnSt_k$,
%    along the inclusion $\dAff_k \to \dSt_k$, together with the pullback functor
%        \[
%            - \times_{(Y\alg)\an} Y \colon \dAnSt_k)_{/ (Y\alg)\an} \to (\dAnSt_k)_{/ Y}, 
%        \]
%    both of which commute with filtered colimits. For the same reason, also the functor $(-)\alg$ commutes with filtered colimits in $X$. 5
%    Therefore, thanks to \cref{rem:reduction_step_closed_ims} we reduce ourselves to prove the statement in the case
%    where $f \colon X \to Y$ is a nil-embedding in $\dAfd_k$.
%
%    Let $f \alg \colon \Spec B \to \Spec A$ be the induced morphism, where $B \coloneqq \Gamma(X, \cO_X\alg)$ and $A \coloneqq \Gamma(Y, \cO_Y\alg)$. It follows readily from \cite[Lemma 6.9]{Porta_Yu_NQK}
%    that the morphism $f\alg$ of derived affine schemes is a nil-isomorphism. Moreover,
%    we have a natural
%    identification
...
%        \end{tikzcd}
%        \]
%    in $\dAnSt_k$. Thus, we obtain a natural morphism $X \to (X\alg)_Y\an$ in $\dAnSt_k$, we wish to prove that such morphism is an equivalence
%    of derived $k$-affinoid spaces. The result now follows immediately from \cite[Proposition 6.11]{Porta_Yu_NQK}.
%\end{proof}
...
%    for $\alpha \in \cI$ where $\cI$ is a filtered category, as follows: approximate $(X_0, Y_0)$ by finite sets $(X_{0, \alpha}, Y_{0, \alpha})$,
%    for $\alpha \in \cJ$ where $\cJ$ is a filtered category. Having defined the pair $(X_{i, \alpha}, Y_{i, \alpha})$ for $i \ge 0$ we let
%        \[
%            (X_{i+1, \alpha}, Y_{i+1, \alpha}),   
%        \]
%    denote the pair of subsets $X_{i+1, \alpha} \subseteq X_{i+1}$ and $Y_{i+1, \alpha} \subseteq Y_{i+1}$ formed by those elements
%    in $X_{i+1}$ (resp., $Y_{i+1}$) whose image via the face maps lie in $X_{i, \alpha}$ (resp., $Y_{i, \alpha}$). In this was, we form
%    a simplicial pair $(X_{\bullet, \alpha}, Y_{\bullet, \alpha})$, indexed by $\alpha \in \cJ$, such that the colimit
%        \[
%            \colim_{\alpha \in \cJ}   (X_{\bullet, \alpha}, Y_{\bullet, \alpha}) \simeq (X_\bullet, Y_\bullet),
%        \]
%    and we are done.
%    In this manner, by taking the respective polynomial resolutions we obtain a filtered diagram
%        \[
...
%\begin{lem} \label{lem:Noetherian_approximation_of_nil_isomorphisms}
%    Let $f \colon X \to Y$ be a nil-embedding in $\dAfd_k$. Then there exists a filtered \infcat $I$ and, for each $\alpha \in I$,
%    nil-embeddings
%        \[
%            f_\alpha \colon X_\alpha \to Y_\alpha  
%        \]
%    in the \infcat $\dAff_k^\laft$, such that the inverse limit
%        \[
%            \lim_{\alpha \in I\op} (f_\alpha \colon X_\alpha \to Y_\alpha)  
%        \]
%    agrees with $f\alg$, in $\dAff_k$.
%\end{lem}
%
%\begin{proof} Let $A \coloneqq \Gamma(X, \cO_X\alg)$ so that $X\alg \simeq \Spec A$. 
%    Since $X\alg \in \dAff_k$ we can find by Noetherian approximation a presentation of the form
%        \[
%            X\alg \simeq \lim_{\alpha \in I} X_\alpha,  
%        \]
%    where $I$ is a filtered \infcat and for each $\alpha \in I$, 
%        \[X_\alpha \coloneqq \Spec A_\alpha \in \dAff_k^\laft,\]
%    for $A_\alpha \in \CAlg_k$ almost of finite presentation derived $k$-algebras. Suppose further that the induced morphisms
%        \[
%            A_\alpha   \to A,
%        \]
%    induce monomorphisms of ordinary rings at the level of $\pi_0(-)$. For each $\alpha \in I$, consider the
%    pushout diagram
%        \[
%        \begin{tikzcd}
%            X \alg \ar{r} \ar{d} & X_\alpha \ar{d} \\
%            Y \alg \ar{r} & Y_\alpha
%        \end{tikzcd},
%        \]
%    computed in the \infcat $\dAff_k$. At the level of derived global sections we obtain a pullback square
%        \[
%        \begin{tikzcd}
%            B_\alpha \ar{r} \ar{d} & B \ar{d} \\
%            A_\alpha \ar{r} & A,
%        \end{tikzcd}
%        \]
%    in the \infcat $\CAlg_k$, where $B \coloneqq \Gamma(Y, \cO_Y\alg)$. The morphism $B_\alpha \to A_\alpha$ is surjective on $\pi_0$.
%    Indeed, it follows from the equivalence of fibers
%        \[
%            \fib(B_\alpha \to A_\alpha) \simeq \fib(B \to A).
%        \]
%    We claim that the morphism $X_\alpha \to Y_\alpha$ is a nil-isomorphism.
%    We start by observing that the induced morphism $\pi_0(B_\alpha) \to \pi_0(B)$ is an injective ring homomorphism, since 
%        \[
%            \fib(B_\alpha \to B) \simeq \fib(A_\alpha \to A) , 
%        \]
%    and $\pi_0(A_\alpha) \to \pi_0(A)$ was assumed to be so.
%    Therefore we have an exact sequence
%        \[
%            0 \to \pi_0(B_\alpha) \to \pi_0(A_\alpha) \oplus \pi_0(B) \to \pi_0(A) \to 0,  
%        \]
%    of discrete $k$-modules.
%    Since the morphism $\pi_0(B_\alpha) \to \pi_0(A_\alpha)$ is surjective so it is the quotient
%        \[
%            \pi_0(B_\alpha)_\red \to \pi_0(A_\alpha)_\red.    
%        \]
%    Moreover, the morphism $\pi_0(B)_\red \to \pi_0(A)_\red$ is an isomorphism of discrete rings. For this reason, it follows that
%    the kernel of the morphism
%       \[
%            \pi_0(A_\alpha)_\red \oplus \pi_0(B)_\red \to \pi_0(A)_\red,
%        \]
%    coincides with $\pi_0(A_\alpha)_\red$ itself. Since the morphism
%    $\pi_0(B_\alpha) \to \pi_0(B)$ is injective (and thus so it is $\pi_0(B_\alpha)_\red \to \pi_0(B)_\red$)
%    we deduce that 
%        \[\pi_0(B_\alpha))_\red \to \pi_0(A_\alpha)_\red \oplus \pi_0(B)_\red,\]
%    is injective, as well. Moreover, the latter must necessarily factor through the injective natural morphism
%        \[
%            \pi_0(A_\alpha)_\red \to \pi_0(A_\alpha)_\red \oplus \pi_0(B)_\red.  
%        \]
%    We thus deduce that $\pi_0(B_\alpha)_\red \simeq \pi_0(A_\alpha)_\red$ and the claim that $X_\alpha \to Y_\alpha$ is nil-isomorphism follows. 
%    
%    It remains to prove that $Y_\alpha \in \dAff^\laft_k$. By our previous considerations,
%    it follows that $\pi_0(B_\alpha)_\red$ is of finite presentation over $k$. The same is thus true for $\pi_0(B_\alpha)$ as the latter can be obtained by a finite sequence
%   of extensions of $\pi_0(B_\alpha)_\red$ by $\pi_0(B_\alpha) \cap \cI$, where $\cI \subseteq \pi_0(B)$ denotes the nilradical of $\pi_0(B)$. Moreover, the latter satisfies
%       \[
%           \cI^n =0,  
%       \]
%    for some sufficiently large $n \ge 0$. The conclusion that $\pi_0(B_\alpha)$ is a finitely presented ordinary $k$-algebra follows.
%   By the derived Hilbert basis theorem, c.f. \cite[Proposition 8.2.5.31]{Lurie_Higher_algebra}, we are left to show that, for each $i>0$,
%    the $\pi_0(B_\alpha)$-modules
%        \[  
%            \pi_i(B_\alpha),
%        \]
%    are finitely presented. But this follows immediately from the existence of exact sequences of finitely presented modules
%        \[
%            \pi_{i+1}(A)\to \pi_i(B_\alpha) \to \pi_i(A_\alpha) \oplus \pi_i(B) \to \pi_i(A),  
%        \]
%    and the claim is thus concluded.
%\end{proof}
...
\end{proof}
    %Let us assume now that $f \colon X \to Y$
...
%    have fiber
%        \[i_n^* \bbL_{(X^{(n)}_\alpha)_\lambda/ Y_\alpha} \in \pro(\Coh(X_\alpha))^{\mathrm{fil}, \le - n}.\] 
%    By assemblying the morphism \eqref{eq:cot_complex_for_alpha} together we deduce that, for each $n \ge 0$, the natural morphism
%        \[
%            \bbL_{X^{\mathrm{alg}, (n)}/ (X_\alpha^{(n)})_\lambda} \to \bbL_{X^{\mathrm{alg}, (n)} / Y_\alpha},
%        \]
%    has fiber equivalent to
%        \[i_n^* \bbL_{X\alg /Y\alg} \in \pro(\Coh(X \alg))^{\mathrm{fil},\le  - n}.\]
%    In particular, we deduce that the fiber at $\lambda \in \bbA^1_k$ of $\colim_{n \ge 0} X^{\mathrm{alg}, (n)}$ can under the morphism
%    the base change $(\theta_{X/ Y})_\lambda$ with
%        \[
%            Y\alg \in \anFMP_{X\alg/ }  ,
%        \]
%    which identifies naturally with $(\cD_{X\alg/ Y\alg})_\lambda$ thanks to \cite[\S 9, Proposition 5.2.2]{Gaitsgory_Study_II} combined with
%    \cref{lem:Noetherian_approximation_of_the_deformation_to_the_normal_bundle}. Suppose now that $\lambda = 0$ in $\bbA^1_k$.
%    In this case, it follows from \cref{lem:Noetherian_approximation_of_nil_isomorphisms} that
%        \[
%            (\cD_{X\alg/ Y\alg}  )_0 \simeq \bbT_{X\alg/ Y\alg}[1]^\wedge.
%        \]
%    It follows from \cite[\S 9, Proposition 5.5.4]{Gaitsgory_Study_II} and the proof of \cite[\S 9, Proposition 5.1.3]{Gaitsgory_Study_II}
%    for the case of vector groups that, for each $n \ge 0$ and each $\alpha \in I$, we have a natural equivalence
%        \[
%              (X_\alpha^{(n)})_0 \simeq \Spec_{X_\alpha}(\Sym^{\le n}(\bbL_{X_\alpha/ Y_\alpha}[-1])),
%        \]
%    in $\mathrm{FMP}_{X_\alpha/ }$. By passing to the limit over $\alpha \in I\op$, we deduce that
%        \[
%            (X^{\mathrm{alg}, (n)})_0 \simeq \Spec_{X\alg}(\Sym^{\le n}(\bbL_{X\alg /Y\alg}[-1])),  
%        \]
%    using \cite[Corollary 4.4.1.3]{Lurie_SAG} combined with \cite[Corollary 4.5.1.3]{Lurie_SAG}. It thus follows that
%        \[  
%            (\theta_{X/ Y})_0 \colon (\colim_{n \ge 0} X^{\mathrm{alg}, (n)})_0 \to \bbT_{X\alg/ Y\alg}[1]^\wedge,
%        \]
%    as desired. The conclusion that $\theta_{X/ Y}$ is an equivalence in the \infcat $\dSt_k$ follows.
%\end{proof}
%    Therefore,
%    by passing to the inverse limit we deduce that the relative cotangent complex of the morphism
%        \[
%            X \to \colim_{n \ge 0} X^{\mathrm{alg}, (n)},
%        \]
%    agrees with $\bbL_{X/ Y} \in \pro(\Coh^(X))$.
%    We have a chain of natural equivalences
%        \begin{align*}
%            (\colim_{n \ge 0} X^{\mathrm{alg}, (n)})_{\lambda} & \simeq \colim_{n \ge 0} (X^{\mathrm{alg}, (n)})_\lambda  \\
%                                                               & \simeq \colim_{n \ge 0} \lim_{\alpha} (X_\alpha^{(n)})_\lambda \\
%                                                               & \simeq \colim_{n \ge 0} \lim_{\alpha}()
%        \end{align*}
%
...
%        \end{equation}
%    We conclude thus that the filtered colimit $\colim_{n \ge 0} X^{\mathrm{alg}, (n)}$ is an affine derived scheme, as well.
